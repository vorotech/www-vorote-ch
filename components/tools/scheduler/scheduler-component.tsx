        ...
        <div className='bg-card rounded-xl border border-border p-6 mb-6 shadow-sm'>
          ...
          <div className='mb-8 p-6 bg-muted/30 rounded-xl border border-border'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
              ...
              <div>
                <label className='block text-sm font-medium text-foreground mb-2'>Start of Week</label>
                <select
                  value={startOfWeek}
              ...
              <div>
                <label className='block text-sm font-medium text-foreground mb-2'>Shift Start Hour (24h)</label>
                <input
                  type='number'
                  min='0'
                  max='23'
              ...
              <div>
                <label className='block text-sm font-medium text-foreground mb-2'>Shift Length</label>
              ...
              {members.map((member) => (
                <div key={member.id} className='mb-6 p-6 bg-card rounded-xl border border-border'>
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1 text-sm rounded transition-colors ${
                              member.allowedWeekdays.length === 0 || member.allowedWeekdays.includes(dayIndex)
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-3'>
                    ...
                    {member.weekendOnly && (
                      <div>
                        <label className='block text-sm text-foreground mb-1'>Max weekend slots per month</label>
                        <input
                          type='number'
                          min='1'
                    ...
                    {member.timeOffs
                      ...
                      .map((timeOff, offIdx) => (
                        <div key={offIdx} className='flex items-center gap-2 mb-2 text-sm'>
                          ...
                          <button onClick={() => removeTimeOff(member.id, offIdx)} className='text-red-500 hover:text-red-700'>
                            Remove
                          </button>
                        </div>
                      ))}
            ...
            <AnimatedGroup className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' preset='blur-slide'>
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className={`p-6 rounded-xl border ${color.bg} ${color.border} hover:shadow-lg transition-all duration-300 hover:border-primary/50 group`}
                  >
              ...
              {viewMode === 'calendar' && (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                    showTimeOff ? 'bg-accent/10 text-primary' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'
                  }`}
                >
                ...
                {schedule.map((slot, idx) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-lg ${
                      isWeekend(slot.date) ? 'bg-muted/50 border border-border' : 'bg-card border border-border'
                    }`}
                  >
                    <div className='flex items-center gap-4'>
                      <span className='font-medium text-foreground w-24 md:w-32 truncate'>{formatDate(slot.date)}</span>
                    ...
                    <div className='flex items-center gap-2'>
                      {slot.member ? (
                        <span
                          className={`px-2 py-1 md:px-4 md:py-1 rounded-full font-medium text-sm md:text-base truncate max-w-[120px] md:max-w-none inline-block align-middle ${getMemberColor(slot.member.id).bg} ${getMemberColor(slot.member.id).text}`}
                        >
                          {slot.member.name}
                        </span>
                      ) : (
                      ...
                      {(() => {
                        ...
                        for (let d = 1; d <= totalDays; d++) {
                          ...
                          cells.push(
                            <div
                              key={d}
                              className={`min-h-[8rem] border-b border-r border-border p-2 hover:bg-muted/30 transition-colors ${
                            ...
                            >
                              ...
                              {slot && slot.member ? (
                                <div
                                  className={`text-xs p-2 rounded border font-medium ${getMemberColor(slot.member.id).bg} ${getMemberColor(slot.member.id).text} ${getMemberColor(slot.member.id).border}`}
                                >
                                  {slot.member.name}
                                  <div className={`text-[10px] mt-1 opacity-75`}>{shiftStartHour.toString().padStart(2, '0')}:00</div>
                                </div>
                              ) : (
                                <div className='text-xs text-muted-foreground italic p-2'>Unassigned</div>
                              )}
                              {showTimeOff && (
                                <div className='mt-1 space-y-1'>
                                  {members
                                    ...
                                    .map((m) => (
                                      <div
                                        key={m.id}
                                        className='flex items-center gap-1 text-[10px] text-muted-foreground bg-muted px-1 rounded border border-border'
                                      >
