...
export const TBody = ({ children }: { children: React.ReactNode }) => {
  return <tbody className='divide-y divide-black/5 dark:divide-white/5'>{children}</tbody>;
};

export const TR = ({ children }: { children: React.ReactNode }) => {
  return <tr className='group transition-colors hover:bg-black/[0.01] dark:hover:bg-white/[0.02]'>{children}</tr>;
};

export const TH = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return <th className={cn('px-6 py-4 font-bold text-black/90 dark:text-white/90', className)}>{children}</th>;
};

export const TD = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return (
    <td
      className={cn('px-6 py-4 font-medium text-black/80 dark:text-white/80 group-hover:text-black dark:group-hover:text-white transition-colors', className)}
    >
      {children}
    </td>
  );
