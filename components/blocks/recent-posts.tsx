    ...
    const fetchPosts = async () => {
      try {
        ...
        if (response.data?.postConnection.edges) {
          const processedPosts = response.data.postConnection.edges.map((postData) => {
        ...
        <div className='container'>
          ...
          {(data.title || data.description) && (
            <div className='text-center mb-12 lg:mb-16'>
              {data.title && (
                <h2 data-tina-field={tinaField(data, 'title')} className='text-3xl font-semibold md:text-4xl lg:text-5xl mb-4'>
                  {data.title}
                </h2>
              )}
              {data.description && (
                <p data-tina-field={tinaField(data, 'description')} className='text-muted-foreground md:text-lg max-w-2xl mx-auto'>
                  {data.description}
                </p>
              )}
            </div>
              ...
              <div className='grid gap-8 lg:grid-cols-2 lg:gap-12'>
                ...
                <div className='flex flex-col justify-center'>
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='text-sm text-muted-foreground'>{featuredPost.published}</div>

                    <Link href={featuredPost.url} className='group inline-flex items-center text-sm font-medium text-primary hover:underline'>
                ...
                <Card key={post.id} className='group overflow-hidden border-0 bg-transparent shadow-none py-0'>
                  ...
                  <div>
                    ...
                    <div className='flex items-center justify-between text-sm'>
                      ...
                      <Link href={post.url} className='group inline-flex items-center text-sm font-medium text-primary hover:underline'>
                        Read more <ArrowRight className='w-4 h-4 ml-1 transition-transform group-hover:translate-x-1' />
                      </Link>
                    </div>
                  </div>
